...
exports.registerKaryawan = exports.loginAkun = exports.addSiswa = void 0;
...
var registerKaryawan = function registerKaryawan(param) {
  var arrKaryawan = param.join("").split(",");
  var objKaryawan = {
    name: arrKaryawan[0],
  ...
  fs.readFile(path, "utf-8", function (err, data) {
    if (err) {
      ...
      fs.writeFile(path, JSON.stringify(realData), function (err) {
        if (err) {
          console.log(err);
        }
...
exports.registerKaryawan = registerKaryawan;

var addSiswa = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(param) {
    var dataParam, namaSiswa, namaMentor, data, realData, indexMentor;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            dataParam = param.join("").split(",");
            namaSiswa = dataParam[0];
            namaMentor = dataParam[1];
            _context.next = 5;
            return _promises["default"].readFile(path);

          case 5:
            data = _context.sent;
            realData = JSON.parse(data); // Get index mentor

            indexMentor = realData.findIndex(function (item) {
              return item.name === namaMentor;
            }); // Add student
...
var loginAkun = function loginAkun(param) {
  var dataParam = param.join("").split(",");
  var username = dataParam[0];
  var password = dataParam[1]; // Read Files

  _promises["default"].readFile(path).then(function (data) {
    var realData = JSON.parse(data); // Cari username

    var indexAkun = realData.findIndex(function (item) {
...
//         })
//         .then(() => console.log("Data Siswa Berhasil Ditambahkan"))
//         .catch((err) => {
//             console.log(err);
//         })
// }
